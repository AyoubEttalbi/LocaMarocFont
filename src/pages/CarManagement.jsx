  ...
  const handleImageChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!file.type.startsWith('image/')) {
        setFormErrors({ image: 'Please upload a valid image file (JPEG, JPG, PNG, or GIF)' });
        return;
      }
      
      // Check file size
      if (file.size > 5120 * 1024) { // 5MB in bytes
        setFormErrors({ image: 'The image must not be larger than 5MB' });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formDataToSend = {
        ...
        gearType: formData.gearType || 'Automatic',
      ...
      if (formData.imageFile && formData.imageFile instanceof File) {
        try {
          ...
          const response = await imageService.uploadImage(formData.imageFile);
          formDataToSend.image = response.imageUrl;
      ...
      // Remove imageFile property as it's not needed in the API call
      delete formDataToSend.imageFile;
      
      // Remove image property if it's null
      ...
      try {
        ...
        if (error.response && error.response.data && error.response.data.errors) {
          ...
          setFormErrors({ general: 'Failed to save car. Please try again.' });
        }
      }
    } catch (error) {
